...
static inline int64_t sbi_call(uint64_t fid, uint64_t arg0, 
    ...
    register long a7 __asm__("a7") = 0;  /* extension ID */
...
static inline void sbi_console_putchar(int ch) {
    sbi_call(SBI_EXT_CONSOLE << 24 | SBI_CONSOLE_PUTCHAR, ch, 0, 0, 0, 0, 0);
}
...
static inline int sbi_console_getchar(void) {
    return (int)sbi_call(SBI_EXT_CONSOLE << 24 | SBI_CONSOLE_GETCHAR, 0, 0, 0, 0, 0, 0);
}
